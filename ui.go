...
<div class="app">
  ...
  <div class="main">
    ...
    <div class="sidebar">
      ...
      <div class="sidebar-section">
        <h4 style="cursor:pointer;" onclick="showBuildModal()" title="Click for details">Build Your Own</h4>
        <div style="font-size:12px;color:var(--text-secondary);line-height:1.7;">
...
<div id="buildModal" style="display:none; position:fixed; inset:0; z-index:1000; background:rgba(0,0,0,0.6); backdrop-filter:blur(4px); display:none; align-items:center; justify-content:center;">
  <div style="background:var(--bg-card); border:1px solid var(--border); border-radius:16px; max-width:520px; width:90%; padding:32px; position:relative; box-shadow:0 25px 50px rgba(0,0,0,0.5);">
    <button onclick="dismissBuildModal()" style="position:absolute; top:12px; right:12px; background:none; border:none; color:var(--text-muted); cursor:pointer; font-size:20px; padding:4px 8px; line-height:1;" title="Close">&times;</button>
    <div style="display:flex; align-items:center; gap:12px; margin-bottom:20px;">
      <div style="background:var(--accent); border-radius:10px; padding:10px; display:flex; align-items:center; justify-content:center;">
        <svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" style="width:22px;height:22px;"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
      </div>
      <h2 style="margin:0; font-size:20px; color:var(--text-primary);">This Is Your Starter Project</h2>
    </div>
    <div style="font-size:14px; color:var(--text-secondary); line-height:1.8;">
      <p style="margin-bottom:14px;">This connector is designed to be forked and extended. Clone the repo, open it with <a href="https://docs.anthropic.com/en/docs/claude-code" target="_blank" style="color:var(--accent); text-decoration:none;">Claude Code</a>, and describe what you need in plain English.</p>
      <div style="background:var(--bg-primary); border-radius:8px; padding:14px 16px; margin-bottom:14px;">
        <div style="font-size:11px; color:var(--text-muted); margin-bottom:6px; text-transform:uppercase; letter-spacing:0.5px;">Example prompts</div>
        <p style="color:var(--text-primary); font-style:italic; margin-bottom:6px;">"Add a Splunk HEC destination to the connector"</p>
        <p style="color:var(--text-primary); font-style:italic; margin-bottom:6px;">"Add a destination that writes to a PostgreSQL table"</p>
        <p style="color:var(--text-primary); font-style:italic; margin-bottom:0;">"Filter out health check logs before exporting to Elasticsearch"</p>
      </div>
      <p style="margin-bottom:14px;">The repo includes a <code style="background:var(--bg-input); padding:2px 6px; border-radius:4px; font-size:13px; color:var(--text-primary);">CLAUDE.md</code> with the full project structure, destination plugin interface, and examples. Claude Code reads it automatically.</p>
      <div style="display:flex; align-items:center; gap:12px; margin-top:18px;">
        <a href="https://github.com/bytefreezer/connector" target="_blank" style="background:var(--accent); color:white; padding:10px 20px; border-radius:8px; text-decoration:none; font-size:14px; font-weight:500; display:inline-flex; align-items:center; gap:6px;">
        ...
        <label style="display:flex; align-items:center; gap:6px; font-size:12px; color:var(--text-muted); cursor:pointer;">
          <input type="checkbox" id="dontShowAgain" style="accent-color:var(--accent);"> Don't show again
        </label>
      </div>
    </div>
  </div>
...
function dismissBuildModal() {
  const m = document.getElementById('buildModal');
  m.style.display = 'none';
  if (document.getElementById('dontShowAgain').checked) {
    localStorage.setItem('bf_connector_hide_build_modal', '1');
  }
}
if (!localStorage.getItem('bf_connector_hide_build_modal')) {
  setTimeout(showBuildModal, 800);
}
